...
from projects.multirobot_searchandrescue.searcher import Searcher
from projects.multirobot_searchandrescue.patrol import get_patrolling_location
# import rospy
...
# Create Simulated data here - instead of using rosy topic subscriber:
SIM = {
...
class SearcherFSM:
    def __init__(self, robot_index):
        ...
        # print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "Initialization successful!")    
    ...
    def cb1_aoa_strength_update(self, data):
        ...
        print(data)
        # print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + 'SIGNAL STRENGTH OF ANOTHER ROBOT UPDATED')
    ...
    def isDemoted(self):
        if (self.S.aoa_strength > self.other_searcher1_aoa_strength) or (self.S.aoa_strength > self.other_searcher2_aoa_strength):
            print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + 'AGENT IS DEMOTED.')
            return True
        else:
            return False
    

    def loop(self, SIM_status, cat=(0,0), mouse=(0,0)):
        while True:
            # by default, stays within the current state
            self.current_state = SIM_status
            ...
            if self.current_state == 'patrolling':
                print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "Patrolling...")
                ...
                print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "Patrolling location goal: " + str(patrolling_location_goal))
                print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "Current robot location:" + str(self.S.get_location()))

                # SIM Assertion
                if len(patrolling_location_goal) != None:
                    return patrolling_location_goal

                # Task 2: move robot to location via move_robot_to_waypoint() - Using SIM
                ...
                if abs(self.S.get_location()[0] - patrolling_location_goal[0]) > WAYPOINT_THRESHOLD[0] or abs(self.S.get_location()[1] - patrolling_location_goal[1]) > WAYPOINT_THRESHOLD[1]:
                    ...
                    print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "Waypoint Script is Finished.")

                # Exit conditions - ROC to SIM
            ...
            if self.current_state == 'listening':
                ...
                # AOA angles should be in global frame, not robot frames - self.S.update_aoa_reading()
                sim_aoa =  self.S.SIM_update_aoa_reading(cat,mouse)

                if sim_aoa != 0:
                    return sim_aoa
                                     
                # Exit conditions - return to SIM
                if self.S.aoa_strength < self.other_searcher1_aoa_strength and self.S.aoa_strength < self.other_searcher2_aoa_strength:
                    next_state = 'hunting'
            
            if self.current_state == 'hunting': # Hunting in SIM
                # print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "Hunting...")
                ...
                print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "is_oriented_to_node: " + str(is_oriented_to_node) + 
            ...
            if(self.current_state != next_state):
                print(SEARCHER_CONFIGS[self.CURRENT_SEARCHER_IDX]['topic'] + ": " + "------ Changing state to: " + next_state + ' ------')                    
