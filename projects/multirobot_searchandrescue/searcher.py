...
from projects.multirobot_searchandrescue.map_to_room_frame import map_to_room_frame, add_barrier_bumper_to_map
...
class Searcher: 
    def __init__(self, name, topic, target_ip_address):
        ...
        # rospy.init_node(name, anonymous=True) # SIM
    ...
    def update_aoa_reading(self):
        ...
        print(self.topic + ": " + "CHECKPOINT 1 of 3")
        aoa_array_msg = rospy.wait_for_message(self.topic+'/wsr_aoa_topic', wsr_aoa_array)
        print(self.topic + ": " + "CHECKPOINT 2 of 3")
        for tx in aoa_array_msg.aoa_array:
            print(self.topic + ": " + "ID: " + tx.id)
            print(self.topic + ": " + "Angle: " + str(tx.aoa_azimuth[0]))
            print(self.topic + ": " + "Variance: " + str(tx.profile_variance))
        ...
        print(self.topic + ": " + "CHECKPOINT 3 of 3")
    ...
    # COMPLETED in SIM
    def SIM_update_aoa_reading(self, cat, mouse):
        # Simply return the distance from mouse and cat to mimic the signal of AOA strength 
    ...
    def obstacle_detected(self):
        ...
        if _distance < SAFE_STOP_DISTANCE and _distance > 0:
            self.stop_robot()
            # print(self.topic + ": " + "OBSTACLE DETECTED. ROBO")
            return True
        return False
    ...
    def get_location(self):
        ...
        # topic_response = rospy.wait_for_message(self.topic + '/mobile_base/odom', Odometry)
        orientation = (SIM['topic_response']['pose']['pose']['position']['x'], 
            SIM['topic_response']['pose']['pose']['position']['y'], 
            SIM['topic_response']['pose']['pose']['position']['z'], 
            SIM['topic_response']['pose']['pose']['position']['w'])
        return orientation

if __name__ == "__main__":
    S = Searcher('searcher1', 'locobot5', '192.168.1.30')
    # print(self.topic + ": " + S.is_moving())
    while True:
        print(S.get_location())
    # print(self.topic + ": " + 'Is target sensed: ' + 'yes' if S.is_target_sensed() else 'no')
