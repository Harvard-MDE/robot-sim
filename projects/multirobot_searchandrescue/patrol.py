...
def get_patrolling_location(SEARCHER_CONFIGS, CURRENT_SEARCHER_IDX):
    ...
    # SIM: generate average choices - SIM map convas 800x800.
    width, height = (800, 800)   
    map = np.array([[1 for x in range(width)] for y in range(height)])
...
def neopolitan(searcher_locations, map):
    ...
    print('Calculating neopolitan_ideals:', neopolitan_ideals)
    
    if len(neopolitan_ideals) == 3:
...
def get_current_searcher_locations(SEARCHER_CONFIGS):
    ...
    for i in range(len(SEARCHER_CONFIGS)):
        ...
        if SIM['on']: # or is_ip_online(SEARCHER_CONFIGS[i]['device_ip']): - no ping in SIM
            # topic_response = rospy.wait_for_message(SEARCHER_CONFIGS[i]['topic'] + '/mobile_base/odom', Odometry, timeout=30)
            searcher_locations[i] = (SIM['topic_response']['pose']['pose']['position']['x'], SIM['topic_response']['pose']['pose']['position']['y'])
        else: 
